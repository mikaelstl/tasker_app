...
import { mockData, createMockId, createMockResponse, createMockTask } from "../../mocks/data";
import type { TaskServiceI } from "./task.service";
...
export class TaskMockService implements TaskServiceI {
  async create(data: CreateTaskDTO): Promise<ApiResponse<TaskDTO>> {
    const task = createMockTask({
      ...
      code: `TSK-${mockData.tasks.length + 1}`,
      name: data.name,
